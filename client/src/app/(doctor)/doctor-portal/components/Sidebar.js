...
const Sidebar = ({ activeStep, setActiveStep, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {/* Navigation */}
      <nav className="mt-4 md:mt-6 flex-1 overflow-y-auto px-2">
          ...
          {navItems.map((item) => (
            <li key={item.step} className="px-2">
              <button
                ...
                className={`w-full flex items-center px-4 py-3.5 text-sm rounded-xl transition-all duration-300 ease-in-out ${
