...
// import { doctors as mockDoctors } from '@/data/mockData'; // Remove old import
...
const DoctorsContent = () => {
  ...
  // Format the combined doctors data
  ...
  const formattedDoctors = combinedDoctorsData.map(doc => ({
    ...
    patients: doc.patients || 0, // Ensure patients exist
                ...
                {filteredDoctors.map((doctor, index) => (
                  <tr key={doctor.id || index}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10 rounded-full overflow-hidden">
                          <img
                            src={doctor.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(doctor.name)}`} // Keep using avatar if passed, else generate
                            alt={doctor.name}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {/* Update links if needed based on ID */}
